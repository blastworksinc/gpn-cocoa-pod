...
static struct CP_GTMClassDescription *GTMClassDescriptions(NSUInteger *total_count) {
  int class_count = objc_getClassList(nil, 0);
  struct CP_GTMClassDescription *class_descs
    = calloc(class_count, sizeof(struct CP_GTMClassDescription));
  if (class_descs) {
...
static void GTMFreeClassDescriptions(struct CP_GTMClassDescription *class_descs,
                                     NSUInteger count) {
...
static NSUInteger CP_GTMGetStackAddressDescriptorsForAddresses(void *pcs[],
                                                            struct CP_GTMAddressDescriptor outDescs[],
                                                            NSUInteger count) {
  ...
  for (NSUInteger i = 0; i < count; ++i) {
    ...
    struct CP_GTMAddressDescriptor *currDesc = &outDescs[i];
...
static NSString *CP_GTMStackTraceFromAddressDescriptors(struct CP_GTMAddressDescriptor descs[],
                                                     NSUInteger count) {
...
//
NSUInteger CP_GTMGetStackProgramCounters(void *outPcs[], NSUInteger count) {
...
NSUInteger CP_GTMGetStackAddressDescriptors(struct CP_GTMAddressDescriptor outDescs[],
                                         NSUInteger count) {
  ...
  NSUInteger newSize = CP_GTMGetStackProgramCounters(pcs, count);

  result = CP_GTMGetStackAddressDescriptorsForAddresses(pcs, outDescs, newSize);
  free(pcs);
  ...
  if (addrCount) {
    ...
    if (pcs) {
      ...
      result = CP_GTMGetStackAddressDescriptorsForAddresses(pcs, outDescs, addrCount);
    }
    free(pcs);
  }
#endif  // MAC_OS_X_VERSION_MIN_REQUIRED >= MAC_OS_X_VERSION_10_5
...
NSString *CP_GTMStackTrace(void) {
  ...
  // infinite loop.
  struct CP_GTMAddressDescriptor descs[100];
  size_t depth = sizeof(descs) / sizeof(struct CP_GTMAddressDescriptor);
  depth = CP_GTMGetStackAddressDescriptors(descs, depth);
  ...
  if (depth > kTracesToStrip) {
    result = CP_GTMStackTraceFromAddressDescriptors(&descs[kTracesToStrip],
  ...
  if (count) {
    void **pcs = calloc(count, sizeof(void*));
    struct CP_GTMAddressDescriptor *descs
      = calloc(count, sizeof(struct CP_GTMAddressDescriptor));
    if (pcs && descs) {
      void **pcsScanner = pcs;
      for (NSNumber *address in addresses) {
        NSUInteger addr = [address unsignedIntegerValue];
      ...
      // Fill in the desc structures
      count = CP_GTMGetStackAddressDescriptorsForAddresses(pcs, descs, count);
      // Build the trace
...
NSString *CP_GTMStackTraceFromException(NSException *e) {
  NSString *trace = @"";
  ...
  if (count) {
    ...
    if (pcs && descs) {
      ...
      count = CP_GTMGetStackAddressDescriptorsForAddresses(pcs, descs, count);
      // Build the trace
      trace = CP_GTMStackTraceFromAddressDescriptors(descs, count);
    }
    free(pcs);
    free(descs);
  }
