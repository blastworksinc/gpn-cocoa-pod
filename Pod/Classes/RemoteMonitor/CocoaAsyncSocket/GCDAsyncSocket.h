...
@class CP_GCDAsyncSocketPreBuffer;
...
extern NSString *const CP_GCDAsyncSocketException;
extern NSString *const CP_GCDAsyncSocketErrorDomain;

extern NSString *const CP_GCDAsyncSocketQueueName;
extern NSString *const CP_GCDAsyncSocketThreadName;

#if SECURE_TRANSPORT_MAYBE_AVAILABLE
extern NSString *const CP_GCDAsyncSocketSSLCipherSuites;
#if TARGET_OS_IPHONE
extern NSString *const CP_GCDAsyncSocketSSLProtocolVersionMin;
extern NSString *const CP_GCDAsyncSocketSSLProtocolVersionMax;
#else
extern NSString *const CP_GCDAsyncSocketSSLDiffieHellmanParameters;
#endif
#endif

enum CP_GCDAsyncSocketError
{
	CP_GCDAsyncSocketNoError = 0,           // Never used
	CP_GCDAsyncSocketBadConfigError,        // Invalid configuration
	CP_GCDAsyncSocketBadParamError,         // Invalid parameter was passed
	CP_GCDAsyncSocketConnectTimeoutError,   // A connect operation timed out
	CP_GCDAsyncSocketReadTimeoutError,      // A read operation timed out
	CP_GCDAsyncSocketWriteTimeoutError,     // A write operation timed out
	CP_GCDAsyncSocketReadMaxedOutError,     // Reached set maxLength without completing
	CP_GCDAsyncSocketClosedError,           // The remote peer closed the connection
	CP_GCDAsyncSocketOtherError,            // Description provided in userInfo
};
typedef enum CP_GCDAsyncSocketError CP_GCDAsyncSocketError;

////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////

@interface CP_GCDAsyncSocket : NSObject
 ...
 * - (dispatch_queue_t)newSocketQueueForConnectionFromAddress:(NSData *)address onSocket:(CP_GCDAsyncSocket *)sock
 * {
 ...
 * Example usage:
 * 
 * - (void)socket:(CP_GCDAsyncSocket *)sock didConnectToHost:(NSString *)host port:(uint16_t)port
 * {
...
@protocol CP_GCDAsyncSocketDelegate
@optional
...
**/
- (void)socket:(CP_GCDAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)socket:(CP_GCDAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)socket:(CP_GCDAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)socket:(CP_GCDAsyncSocket *)sock shouldTimeoutReadWithTag:(long)tag
...
**/
- (NSTimeInterval)socket:(CP_GCDAsyncSocket *)sock shouldTimeoutWriteWithTag:(long)tag
                                                                  elapsed:(NSTimeInterval)elapsed
                                                                bytesDone:(NSUInteger)length;
...
**/
- (void)socketDidCloseReadStream:(CP_GCDAsyncSocket *)sock;
...
**/
- (void)socketDidSecure:(CP_GCDAsyncSocket *)sock;
