...
NSString *const CP_GCDAsyncSocketException = @"CP_GCDAsyncSocketException";
NSString *const CP_GCDAsyncSocketErrorDomain = @"CP_GCDAsyncSocketErrorDomain";

NSString *const CP_GCDAsyncSocketQueueName = @"CP_GCDAsyncSocket";
NSString *const CP_GCDAsyncSocketThreadName = @"CP_GCDAsyncSocket-CFStream";

#if SECURE_TRANSPORT_MAYBE_AVAILABLE
NSString *const CP_GCDAsyncSocketSSLCipherSuites = @"CP_GCDAsyncSocketSSLCipherSuites";
#if TARGET_OS_IPHONE
NSString *const CP_GCDAsyncSocketSSLProtocolVersionMin = @"CP_GCDAsyncSocketSSLProtocolVersionMin";
NSString *const CP_GCDAsyncSocketSSLProtocolVersionMax = @"CP_GCDAsyncSocketSSLProtocolVersionMax";
#else
NSString *const CP_GCDAsyncSocketSSLDiffieHellmanParameters = @"CP_GCDAsyncSocketSSLDiffieHellmanParameters";
#endif
#endif

enum CP_GCDAsyncSocketFlags
{
...
@interface CP_GCDAsyncSocket ()
{
	...
	CP_GCDAsyncSocketPreBuffer *preBuffer;
	...
	CP_GCDAsyncSocketPreBuffer *sslPreBuffer;
...
@interface CP_GCDAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation CP_GCDAsyncSocketPreBuffer
...
- (NSUInteger)readLengthForTermWithPreBuffer:(CP_GCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;
...
@implementation CP_GCDAsyncSocket
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([CP_GCDAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[CP_GCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
	}
	return self;
}
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			CP_GCDAsyncSocket *acceptedSocket = [[CP_GCDAsyncSocket alloc] initWithDelegate:theDelegate
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:CP_GCDAsyncSocketErrorDomain code:CP_GCDAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:CP_GCDAsyncSocketErrorDomain code:CP_GCDAsyncSocketBadParamError userInfo:userInfo];
}
...
- (NSError *)connectTimeoutError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"CP_GCDAsyncSocketConnectTimeoutError",
	                                                     @"CP_GCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Attempt to connect to host timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:CP_GCDAsyncSocketErrorDomain code:CP_GCDAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"CP_GCDAsyncSocketReadMaxedOutError",
														 @"CP_GCDAsyncSocket", [NSBundle mainBundle],
														 @"Read operation reached set maximum length", nil);
	
	NSDictionary *info = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:CP_GCDAsyncSocketErrorDomain code:CP_GCDAsyncSocketReadMaxedOutError userInfo:info];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"CP_GCDAsyncSocketReadTimeoutError",
	                                                     @"CP_GCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Read operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:CP_GCDAsyncSocketErrorDomain code:CP_GCDAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"CP_GCDAsyncSocketWriteTimeoutError",
	                                                     @"CP_GCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Write operation timed out", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:CP_GCDAsyncSocketErrorDomain code:CP_GCDAsyncSocketWriteTimeoutError userInfo:userInfo];
}

- (NSError *)connectionClosedError
{
	NSString *errMsg = NSLocalizedStringWithDefaultValue(@"CP_GCDAsyncSocketClosedError",
	                                                     @"CP_GCDAsyncSocket", [NSBundle mainBundle],
	                                                     @"Socket closed by remote peer", nil);
	
	NSDictionary *userInfo = [NSDictionary dictionaryWithObject:errMsg forKey:NSLocalizedDescriptionKey];
	
	return [NSError errorWithDomain:CP_GCDAsyncSocketErrorDomain code:CP_GCDAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:CP_GCDAsyncSocketErrorDomain code:CP_GCDAsyncSocketOtherError userInfo:userInfo];
}
...
{
	CP_GCDAsyncSocket *asyncSocket = (__bridge CP_GCDAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	#if TARGET_OS_IPHONE
	{
		...
		NSString *sslMinLevel = [tlsSettings objectForKey:CP_GCDAsyncSocketSSLProtocolVersionMin];
		NSString *sslMaxLevel = [tlsSettings objectForKey:CP_GCDAsyncSocketSSLProtocolVersionMax];
		...
		if (sslLevel)
		{
			if (sslMinLevel || sslMaxLevel)
			{
				LogWarn(@"kCFStreamSSLLevel security option ignored. Overriden by "
						@"CP_GCDAsyncSocketSSLProtocolVersionMin and/or CP_GCDAsyncSocketSSLProtocolVersionMax");
			}
			else
			{
	...
	value = [tlsSettings objectForKey:CP_GCDAsyncSocketSSLCipherSuites];
	if (value)
	{
	...
	#if !TARGET_OS_IPHONE
	value = [tlsSettings objectForKey:CP_GCDAsyncSocketSSLDiffieHellmanParameters];
	if (value)
	{
	...
	sslPreBuffer = [[CP_GCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
{
	[[NSThread currentThread] setName:CP_GCDAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(CP_GCDAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(CP_GCDAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	CP_GCDAsyncSocket *asyncSocket = (__bridge CP_GCDAsyncSocket *)pInfo;
